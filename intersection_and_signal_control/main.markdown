...
We should remember that a crossing for access will be required to curb-side mid-block stations as well.
...
![equation-capacity-definition](img/eq-cap-flow "CapFlow = saturationflowperlane * NLanes * Time_green/Time_cycle")
...
Time_green / Time_cycle: relative (effective) green time, that we may also call K_green
...
saturationflowperlane * NLanes_away * 1.0 >= saturationflowperlane * NLanes_intersection * Time_green-away/ Time_cycle-away
...
- If we are planning a mid-block station with signalized crossing access, the green time for traffic mid-block over the cycle time has to be included in the equation:

NLanes_away >= NLanes_intersection * 
...
The introduction or existence of flyovers or underpasses in the corridor requires special treatments for BRT systems and they represent an opportunity to dramatically improve BRT vehicle operation.  Exclusive busway use of a flyover is a successful technique used in several existing BRT systems despite its potential major adverse visual impacts, which are highly undesirable from an aesthetics standpoint, in cities and urban areas.

Thus, the first option is to consider grade separation infrastructure to be dedicated to BRT usage.
...
If the grade-separated design solution maintains BRT lane through intersections, same concerns about queue blocking the stations apply for the traffic signal times on surface level. 
...
A second possibility is when the flyover is built on the road perpendicular to the BRT corridor.  In this case, the flyover does not introduce any special difficulty and can help to alleviate  a BRT intersection and increase green phase time.
...
Figure 9.16 Schematic of Delhi roadway in which BRT vehicles must cross mixed traffic that is attempting to access a flyover.

A third possibility is to construct two separate flyovers, one for each traffic direction, leaving a space between the flyover for the BRT system that allows BRT buses to continue along the surface. 

Figure New draw for 3rd possibility.

A fourth possibility is to allow the BRT vehicles to also pass over the flyover, either in a segregated lane, or if there is no space for a segregated lane, then in mixed traffic.  This last option sometimes results in stations being far from intersections, which may produce inconvenience for passengers seeking to reach destinations near the intersection.  Additionally, this configuration can be particularly problematic if there is a connecting BRT corridor running on the perpendicular street below the flyover or above the underpass.  
